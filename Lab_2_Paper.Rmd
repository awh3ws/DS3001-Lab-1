...
Similarly, a different linear model system that also included interaction variables managed to produce an `R^2` value of `0.0865`, however, the variables interacted were completely different. `Age` was interacted with `average_glucose_level` which was then interacted with `smoking_status`. The fact that two separate and similar `R^2` values were generated through completely different means indicates that there is some correlation between some of the variables, and that the variables chosen for interactions are predictors of strokes.
...
Our best overall model was a linear model that included bins for most variables, and interacted age, average glucose level, and smoking status. The resulting R^2 is 0.0865.

```{r}
df_test = read.csv("testing_data.csv")
df_train = read.csv("training_data.csv")

df_train = df_train[-c(1,2)] #Removing bad vectors that perfectly predict stroke
df_test = df_test[-c(1,2)]

df_train$bmi = as.numeric(df_train$bmi) #Converting BMI to numeric
df_test$bmi = as.numeric(df_test$bmi)

df_train = na.omit(df_train) #Omitting all NAs
df_test = na.omit(df_test)

df_train$gender = ifelse(df_train$gender == "Male", 1, 0) #Converting character dummy variables to numerics
df_train$ever_married = ifelse(df_train$ever_married == "Yes", 1.0, 0)
df_train$Residence_type = ifelse(df_train$Residence_type == "Rural", 0.0, 1)
df_train$smoked = ifelse(df_train$smoking_status == "formerly smoked" | df_train$smoking_status == "smokes", 1.0, 0) #This choice is explained in the data section

df_test$gender = ifelse(df_test$gender == "Male", 1, 0)
df_test$ever_married = ifelse(df_test$ever_married == "Yes", 1.0, 0)
df_test$Residence_type = ifelse(df_test$Residence_type == "Rural", 0.0, 1)
df_test$smoked = ifelse(df_test$smoking_status == "formerly smoked" | df_test$smoking_status == "smokes", 1.0, 0)

df_train$gluc = ifelse(df_train$avg_glucose_level > 190, 1, 0) #Binning variables based on tree models, kNN, kMC, and trial and error
df_train$old = ifelse(df_train$age > 69, 1 , 0)
df_train$overbmi = ifelse(df_train$bmi > 20, 1, 0)
df_train$og = ifelse(df_train$old == 1 & df_train$gluc == 1, 1, 0) #Interacting variables
df_train$b = ifelse(df_train$og == 1 & df_train$smoked == 1, 1, 0)



df_test$gluc = ifelse(df_test$avg_glucose_level > 190, 1, 0)
df_test$old = ifelse(df_test$age > 69, 1 , 0)
df_test$overbmi = ifelse(df_test$bmi > 20, 1, 0)
df_test$og = ifelse(df_test$old == 1 & df_test$gluc == 1, 1, 0)
df_test$b = ifelse(df_test$og == 1 & df_test$smoked == 1, 1, 0)


model = lm(stroke~.,data=df_train) #Model generation
test = predict(model, df_test) #Testing the model


y_bar = mean(df_train$stroke)
y_test = df_test$stroke
TSS = sum( (y_test - y_bar)^2 ) 

rtest = 1 - sum( (y_test - test)^2 )/TSS
print(rtest)

plot(test, df_test$stroke)
```

As seen in the plot, this predictor is still not perfect, but gets closer than any other attempt.

## Conclusion
	In conclusion, all of our models fell short of the `R^2` of `.1569` given in the assignment description. Using K nearest neighbor, K means squared, and linear models to create a model, we found that linear models were the most effective model to achieve a high `R^2` with an `R^2` of `0.0865`. Since the assignment asked us to create a model to predict the chance of strokes, we believe that our model is not safe or probable enough to be implemented in real life. A very low R^2 shows that the model is not a good predictor and if implemented, we would probably have many type 1 (false positive) and type 2 (false negative) errors. Therefore, we have failed to create a model that should be a good predictor and implemented in real life. Given a dataset of this size, always choosing that the individual will not have a stroke results in a reasonably high probability that the choice was correct. For this reason, a larger dataset with more individuals who suffered from strokes could drastically increase the efficiency of our models. Future research on this topic should include a more comprehensive dataset that allows for more conclusions to be drawn about stroke victims. Furthermore, utilization of a random forest might produce a high `R^2` as most variables in our best model were divided into binary bins, indicating that some form of decision tree could produce good results. Our approach in determining the highest `R^2` is highly defensible as up until the last step of trial and error, almost all of our choices were decided by the data. We utilized various forms of prediction and grouping models to determine which variables are correlated with strokes, and what the values are that best predict a stroke. We then used these variables in a variety of ways to generate linear models. Finally, we slightly tweaked the bin divisions to achieve the maximum `R^2`. Obviously, there will be a higher possible `R^2` that can be obtained through more interactions, more binning, or a different model entirely. Finding this value was not feasible as we do not have the tools or desire to game an `R^2` that is `0.001` higher out of the data. We feel that our result of `0.0865` is the maximum possible value that we can achieve with this given dataset at this point in time as finding this value already required the use of all of oiur tools.
