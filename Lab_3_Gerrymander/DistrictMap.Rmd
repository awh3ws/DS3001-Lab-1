...
for (i in 1:11){
  for (j in 1:length(districts[[i]])){
    for (k in 1:length(d$fips)){
     if(toupper(districts[[i]][[j]]) == gsub(" ", "", votingVA$county_name[k]))
       districts[[i]][[j]] = votingVA$county_fips[k]
